...
|---|---|---|---|
| Goodnotes | **구현 완료, 앱 왕복 검증 대기** (2026-08-31 갱신, 아래 «Goodnotes 구현 결과» 참고) | `.goodnotes` 문서는 다시 가져올 수 있는 편집 가능한 백업이지만, 확인한 공식 문서는 내부 펜 데이터·배경 PDF·쪽 대응 구조를 공개하지 않는다. PDF 백업은 앱에서 같은 방식으로 편집할 수 없다고 명시한다. | 실제 `.goodnotes` 원본/왕복 샘플에서 배경 교체, 쪽 재정렬, 펜 원시 데이터 보존을 구조·앱 양쪽에서 검증 |
...
## Goodnotes 구현 결과 (2026-08-31)

이 조사가 요구한 실제 샘플이 확보되어 **읽기·미리보기·저장을 구현했다**. 아래는 이 문서의
2026-08-30 판정을 무엇이 바꿨는지다.

### 무엇이 바뀌었나

- 2026-08-30 판정의 전제였던 "공식 문서만으로는 내부 구조를 확정할 수 없다"는 여전히 맞지만,
  그 사이 두 MIT 프로젝트가 **역공학 결과와 실제 내보내기 파일을 공개**했다. 그중
  `cable729/inkterop` 의 `docs/formats/goodnotes.md` 와 통제된 Mac 내보내기 파일
  `gn-mac-mixed-pens.goodnotes` 를 받아 `tests/fixtures/goodnotes/` 에 두고 기준으로 삼았다.
- 같은 문서에 **동일 형식 왕복이 Goodnotes Mac 앱에서 실제로 열리고 필기가 그대로 그려졌다는
  기록**(2026-07-09, 스크린샷 근거)이 있다. NotEditor가 필요한 것이 정확히 "같은 형식 왕복 +
  배경 교체"라서, 이 조사가 요구한 통과 조건 중 가장 중요한 항목이 충족됐다.
- 반대로 **다른 형식에서 합성한 `.goodnotes` 는 여전히 앱이 거절**한다. 그래서 NotEditor는
  획을 새로 만들지 않고 원본 저널을 그대로 옮기는 경로만 쓴다.

### 우리가 직접 확인한 사실

실제 파일을 NotEditor 코드로 읽어 확인했다(문서만 보고 옮겨 적은 것이 아니다).

- 컨테이너는 ZIP이고 `schema.pb` 는 `08 19` — 스키마 25.
- 문서의 진짜 목차는 `index.events.pb` 다. 용지 기록(최상위 필드 2)이 배경 첨부와 그 안의 쪽
  번호·캔버스 크기를, 쪽 생성(54)이 쪽 개체와 용지 참조·정렬 키를, 쪽 연결(105)이 쪽 번호와
  쪽 내용 ID를 들고 있다.
- **쪽 개체 ID = 쪽 내용 ID − 1** 인접 규칙이 실제 파일에서 성립한다. 필기 `notes/<내용 ID>` 를
  쪽에 잇는 유일한 실마리라 저장할 때도 지킨다.
- 획 좌표는 Apple 프레임 LZ4(`bv41`) 안의 `tpl` 블록이고, 블록 앞의 ASCII 서명이 뒤 구역들의
  형태를 알려준다. 세 서명 계열(압력 펜·일정 굵기·연필)이 모두 관측됐고, 우리 디코더가 푼
  크기가 프레임이 예고한 크기와 정확히 일치한다.
- 해석한 획을 그려 **아카이브 자체의 `thumbnail.jpg`(앱이 그린 그림)와 대조**했다. 펜 6획의
  위치·색·굵기가 일치하고, 기본 굵기도 문서화된 값과 같다(볼펜 1.56pt, 형광펜 24pt, 마커 18pt).
  도형(타원) 하나는 그리지 않는다 — 알려진 미지원 객체다.

### 아직 확인하지 못한 것

- **저장한 `.goodnotes` 를 Goodnotes 앱에서 열어 보지 못했다.** 개발 환경에 macOS·iPad가 없다.
  이 조사가 요구한 통과 조건 4(재가져오기 뒤 획 지우기·올가미 이동·재정렬)는 미충족이다.
- 실제 파일이 **한 쪽짜리, 기본 용지(가져온 PDF가 아님) 문서 하나뿐**이다. 여러 쪽 문서와
  PDF를 가져와 만든 문서는 같은 코드 경로를 타지만 실제 파일로 확인하지 못했다.
- 스키마 24(iPad 시절) 파일은 획 읽기는 같은 경로를 타지만 실제 파일로 확인하지 못했다.
- Apple과 Android·Windows의 `.goodnotes` 가 같은 구조라는 증거는 여전히 없다.

### 구현 위치

`noteditor/goodnotes_proto.py`(바이트 계층), `goodnotes_archive.py`(쪽 구조와 사건 기록
다시 쓰기), `goodnotes_ink.py`(미리보기), `goodnotes_transfer.py`(진단·미리보기·저장).
검사는 `tests/test_goodnotes_transfer.py` 가 실제 파일을 기준으로 돌린다.
