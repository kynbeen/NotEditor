...
5mm 이상이면 모든 대응 쪽을 확인 필요로 표시합니다. 미리보기에서 각 대응을 확인하거나
저장 확인창에서 명시적으로 승인한 뒤 저장할 수 있습니다.
...
`noteditor.cmd`는 **반드시 저장소 루트에 그대로 두세요.** Sleek이 `shutil.which("noteditor")`
...
Sleek에서 `합치기`를 누르면 NotEditor가 지정된 로컬 수집함을 시작 경로로 바로 열어 여러
PDF의 쪽과 순서를 고르게 합니다. 이미 사용한 PDF의 수집함 원본이 바뀌면 실제 사용본과 현재
원본을 같은 쪽·다른 쪽 두 열로 비교하고, 온전한 파일은 `전체 갱신`, 합친 파일은 `합쳐서 갱신`,
기존 파일을 유지하려면 `넘어가기` 결정을 Sleek에 돌려줍니다.

```powershell
.\venv\Scripts\pythonw.exe -m noteditor --open-plan C:\path\to\handoff.json
```

새 인계 계획은 workspace의 계약 판 3을 따르며 기존 판 1·2 합치기 계획도 계속 읽습니다. 합치기
결과는 계획의 `output_path`에 PDF를 먼저 기록하고 바로 옆에 `<결과>.merge.json` 사이드카를
남깁니다. 원본 검토 결정은 결과가 필요한 경우 그 저장까지 끝난 뒤 별도 결정 파일에 원자적으로
기록합니다. 판 3 결정은 파일 교체 방식과 변경 종류를 분리하고, 문제 변경이면 바뀐 쪽 번호도
함께 기록합니다. 기준 파일과 선택 대상은 지정된 로컬 수집함 경계 안에서만 다루며 Drive 파일을 직접
열거나 수정하지 않습니다. 일반 실행과 웹 실행은 이 옵션을 사용하지 않고 기존 저장 흐름을 유지합니다.

인계로 열린 창은 **그 한 가지 일만** 합니다. `필기 옮기기` 탭은 잠기고, 저장 버튼은
`저장하고 Sleek으로 돌아가기`가 되며, 누르면 결과를 남긴 뒤 **이 창이 스스로 닫힙니다**
(`ComposerApi.close_window` — 인계로 열린 창에서만 동작합니다). 화면을 되찾는 것은
Sleek 쪽입니다. 원본 비교 화면은 창 전체가 세로로 스크롤되고 쪽 대조는 세 쌍이 한
화면에 들어가는 높이로 그립니다. `다른 쪽` 버튼으로 달라진 곳만 건너뛸 수 있고, 합친
...
고르다 결과 순서가 `A…B…A`가 되면 재현할 수 없습니다. Sleek 인계 화면은 문서·쪽 번호의
기본 순서를 유지하고 순서 드래그를 제공하지 않아, 화면에서는 가능해 보이지만 저장할 때 거절되는
모순을 막습니다. 일반 NotEditor 합치기는 이 계약 밖이므로 자유 순서와 새 쪽을 같은 문서 옆에
넣는 동작(`insertNearOwnPages`)을 그대로 제공합니다.
