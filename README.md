...
`저장하고 summary.ai로 돌아가기`가 되며, 누르면 결과를 남긴 뒤 **이 창이 스스로 닫힙니다**
(`ComposerApi.close_window` — 인계로 열린 창에서만 동작합니다). 화면을 되찾는 것은
summary.ai 쪽입니다. 원본 비교 화면은 창 전체가 세로로 스크롤되고 쪽 대조는 세 쌍이 한
...
고르다 결과 순서가 `A…B…A`가 되면 재현할 수 없습니다. summary.ai 인계 화면은 문서·쪽 번호의
