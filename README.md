...
브라우저는 `HttpOnly` 쿠키로 자기 작업공간을 가리킵니다.
...
- **자동 정리**: 목록에서 뺀 문서는 사본까지 지우고, 등록에 실패한 업로드도 남기지 않습니다.
  비활성 작업공간은 `NOTEDITOR_SWEEP_INTERVAL`마다 통째로 사라집니다.
- **수동 정리**: 상단의 `작업공간 비우기` 버튼이 임시 폴더를 지우고 새로 엽니다.
