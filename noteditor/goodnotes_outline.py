...
def map_outline_to_result(
    ...
    for entry_index, entry in enumerate(entries, start=1):
        ...
        if len(matches) != 1:
            label = "새 PDF" if basis == PAGE_BASIS_TARGET else "원본 Goodnotes"
            raise GoodnotesTransferError(
                f"목차 {entry_index}번의 {label} {entry.page}쪽이 결과에 없거나 중복됩니다. "
                "쪽 제외와 목차 쪽 번호 기준을 확인하세요."
            )
    ...
    try:
        for event in split_delimited(events):
            for event_type, wire_type, value in iter_fields(event):
                ...
                # build_events emits these event types. Counter locations differ
                # by type; numeric magnitude cannot distinguish an actor ID.
                counter_field = {30: 14, 6: 15, 2: 16, 54: 14, 105: 14}.get(event_type)
                counters.extend(
                    int(value) for value in fields.get(counter_field, ())
